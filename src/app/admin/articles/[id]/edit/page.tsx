...
import { Article, Category, DetailArticles, EditArticles, listCategory, UploadImage } from "@/api/admin";
...
export default function ArticleAdmin() {
    ...
    useEffect(() => {
        ...
        const formatted = htmlToPlainText(detailData?.content);
    ...
    return showPreview ?
        <AuthGuard allowedRole="Admin">
            <Preview content={plainText} title={detailData?.title} image={image} handleBack={() => setShowPreview(false)} />
        </AuthGuard>
        :
        <AuthGuard allowedRole="Admin">
                ...
                <main className="w-full">
                    <nav className="flex justify-between py-5 px-7 bg-white">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <p onClick={() => router.push('/admin/profile')} className="hidden md:block underline text-black font-bol cursor-pointer">{profileData?.username}</p>
                        </div>
                    </nav>

                    <div className="md:px-10 mt-10 ">
                        <div className="bg-white pb-1">
                            <p className="py-5 px-7 font-semibold flex gap-2 items-center text-lg"><ArrowLeft className="cursor-pointer" onClick={() => router.back()} /> Edit Article</p>
                            <hr />
                            ...
                            <div className="grid w-full  items-center gap-2 px-5 mt-6">
                                ...
                                <Input
                                    ...
                                    onChange={(e) => {
                                        setDetailData({ ...detailData, title: e.target.value });
                                    }}
                                    id="title"
                                    type="title "
                            ...
                            <div className="grid w-full  items-center gap-2 px-5 mt-6">
                                ...
                                <Select onValueChange={(e) => setDetailData({ ...detailData, category: { ...detailData.category, id: e } })} value={detailData?.category?.id}>
                            ...
                            <div className="grid w-full items-center gap-2 mt-6 px-5">
                                <div className="border-[1px] rounded-xl">
                                    ...
                                    <Textarea className="border-0 shadow-none" value={plainText} onChange={(e) => setDetailData({ ...detailData, content: e.target.value })} />
                                </div>
                                {detailData?.content == "" &&
                            ...
                            <div className="flex justify-end my-14 px-6 gap-2">
                                ...
                                <Button disabled={!detailData?.content || !detailData?.category.id || !detailData.title || !image || loading} variant={'secondary'} onClick={() => setShowPreview(true)}>Preview</Button>
                                <Button
                                    onClick={handleEdit}
                                    disabled={!detailData?.content || !detailData?.category.id || !detailData.title || !image || loading} variant={'primary'}>Upload</Button>
                            </div>
                        </div>
                    </div>
                </main>
